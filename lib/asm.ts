...
export const ARCH_X86 = 4
export const MODE_16 = 2
export const MODE_32 = 4
export const MODE_64 = 8
...
class Asm {
    ...
    load(code: string) {
        ...
        if (Keystone.ks_asm(this._ks, code, 0, encoding, size, count) !== 0) {
            throw new Error('Error: failed on load()')
        }

        return Buffer.from(ref.reinterpret(encoding.deref(), size.deref(), 0))
    }

    setOption(type: number, value: number) {
